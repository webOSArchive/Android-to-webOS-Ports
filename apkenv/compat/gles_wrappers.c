...
/* Currently-bound draw framebuffer (0 = the real screen). The screen-rotation
 * hacks (glOrthof rotate, glViewport/glScissor swap) must apply ONLY when
 * rendering to the real screen — NOT while rendering into an offscreen FBO
 * (render-to-texture), else the intermediate content gets wrongly rotated and
 * the composited world ends up sideways. Set by my_glBindFramebuffer{,OES}. */
int apkenv_bound_fbo = 0;

/* ===================================================================== *
 * Stage 3: render-to-portrait-FBO (replaces the per-call rotation hooks)  *
 *                                                                         *
 * The engine renders its WHOLE frame (world, UI, fluid, every offscreen   *
 * render-to-texture pass) into one offscreen portrait FBO, un-rotated. We *
 * then present that FBO to the native landscape framebuffer with ONE      *
 * 90deg rotated textured-quad blit. This keeps the engine's entire        *
 * internal frame self-consistent portrait, which the per-glOrthof hooks    *
 * could not do across separate render passes (the WMW water bug).         *
 * Enabled by module_hacks->render_to_fbo; size = fbo_w x fbo_h.           *
 * ===================================================================== */
static GLuint apkenv_fbo_id = 0, apkenv_fbo_tex = 0, apkenv_fbo_depth = 0;
static int apkenv_fbo_ready = 0;
static int apkenv_fbo_rot = -1;   /* 0..3 = 0/90/180/270 deg; -1 = read env */

static void
apkenv_fbo_ensure(void)
{
    if (apkenv_fbo_ready || !global_module_hacks.render_to_fbo)
        return;
    int w = global_module_hacks.fbo_w, h = global_module_hacks.fbo_h;
    if (w <= 0 || h <= 0)
        return;

    if (apkenv_fbo_rot < 0) {
        const char *r = getenv("APKENV_WMW_FBO_ROT");
        apkenv_fbo_rot = r ? (atoi(r) & 3) : 1;   /* default 90deg */
    }

    functions.glGenTextures(1, &apkenv_fbo_tex);
    functions.glBindTexture(GL_TEXTURE_2D, apkenv_fbo_tex);
    functions.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0,
    ...
    functions.glGenRenderbuffersOES(1, &apkenv_fbo_depth);
    functions.glBindRenderbufferOES(GL_RENDERBUFFER_OES, apkenv_fbo_depth);
    functions.glRenderbufferStorageOES(GL_RENDERBUFFER_OES,
                                       GL_DEPTH_COMPONENT16_OES, w, h);

    functions.glGenFramebuffersOES(1, &apkenv_fbo_id);
    functions.glBindFramebufferOES(GL_FRAMEBUFFER_OES, apkenv_fbo_id);
    functions.glFramebufferTexture2DOES(GL_FRAMEBUFFER_OES, GL_COLOR_ATTACHMENT0_OES,
                                        GL_TEXTURE_2D, apkenv_fbo_tex, 0);
    functions.glFramebufferRenderbufferOES(GL_FRAMEBUFFER_OES, GL_DEPTH_ATTACHMENT_OES,
                                           GL_RENDERBUFFER_OES, apkenv_fbo_depth);

    apkenv_fbo_ready = 1;
    fprintf(stderr, "[FBO] portrait FBO ready %dx%d id=%u tex=%u rot=%d\n",
            w, h, apkenv_fbo_id, apkenv_fbo_tex, apkenv_fbo_rot);
    /* leave our FBO bound: the engine's fb-0 binds are redirected here */
}
...
apkenv_fbo_present(void)
{
    ...
    int sw = 0, sh = 0;
    global.platform->get_size(&sw, &sh);   /* native landscape, e.g. 1024x768 */

    /* ---- save engine GL state we are about to clobber, so the engine's own
     * per-frame setup AND its between-frame async work (texture uploads, the
    ...
    GLboolean e_depth = functions.glIsEnabled(GL_DEPTH_TEST);
    GLboolean e_cull  = functions.glIsEnabled(GL_CULL_FACE);
    GLboolean e_blend = functions.glIsEnabled(GL_BLEND);
    GLboolean e_light = functions.glIsEnabled(GL_LIGHTING);
    GLboolean e_fog   = functions.glIsEnabled(GL_FOG);
    GLboolean e_alpha = functions.glIsEnabled(GL_ALPHA_TEST);
    GLboolean e_scis  = functions.glIsEnabled(GL_SCISSOR_TEST);
    GLboolean e_tex   = functions.glIsEnabled(GL_TEXTURE_2D);
    GLboolean a_vert  = functions.glIsEnabled(GL_VERTEX_ARRAY);
    GLboolean a_texc  = functions.glIsEnabled(GL_TEXTURE_COORD_ARRAY);
    GLboolean a_col   = functions.glIsEnabled(GL_COLOR_ARRAY);
    GLboolean a_norm  = functions.glIsEnabled(GL_NORMAL_ARRAY);

    /* TRIANGLE_STRIP fullscreen quad in NDC: BL, BR, TL, TR */
    static const GLfloat verts[8] = { -1,-1,  1,-1,  -1,1,  1,1 };
    /* 4 texcoord rotations (tex origin bottom-left). 90/270 preserve aspect
     * (portrait 768x1024 tex onto landscape 1024x768 screen). */
    ...
    functions.glBindFramebufferOES(GL_FRAMEBUFFER_OES, 0);  /* real window FB */
    functions.glViewport(0, 0, sw, sh);
    functions.glDisable(GL_DEPTH_TEST);
    functions.glDisable(GL_CULL_FACE);
    functions.glDisable(GL_BLEND);
    functions.glDisable(GL_LIGHTING);
    functions.glDisable(GL_FOG);
    functions.glDisable(GL_ALPHA_TEST);
    functions.glDisable(GL_SCISSOR_TEST);
    functions.glActiveTexture(GL_TEXTURE0);
    functions.glClientActiveTexture(GL_TEXTURE0);
    functions.glEnable(GL_TEXTURE_2D);
    functions.glBindTexture(GL_TEXTURE_2D, apkenv_fbo_tex);
    functions.glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
    functions.glColor4f(1.f, 1.f, 1.f, 1.f);
    ...
    /* rebind our FBO so the engine's next frame renders into it again */
    functions.glBindFramebufferOES(GL_FRAMEBUFFER_OES, apkenv_fbo_id);

    /* ---- restore every piece of engine GL state we touched. Restoring the
     * saved viewport also fixes the "engine sets viewport only at init" case
     * (it gets back its real 768x1024 FBO viewport). ---- */
    functions.glViewport(s_vp[0], s_vp[1], s_vp[2], s_vp[3]);
    if (e_depth) functions.glEnable(GL_DEPTH_TEST); else functions.glDisable(GL_DEPTH_TEST);
    if (e_cull)  functions.glEnable(GL_CULL_FACE);  else functions.glDisable(GL_CULL_FACE);
    if (e_blend) functions.glEnable(GL_BLEND);      else functions.glDisable(GL_BLEND);
    if (e_light) functions.glEnable(GL_LIGHTING);   else functions.glDisable(GL_LIGHTING);
    if (e_fog)   functions.glEnable(GL_FOG);        else functions.glDisable(GL_FOG);
    if (e_alpha) functions.glEnable(GL_ALPHA_TEST); else functions.glDisable(GL_ALPHA_TEST);
    if (e_scis)  functions.glEnable(GL_SCISSOR_TEST); else functions.glDisable(GL_SCISSOR_TEST);
    if (e_tex)   functions.glEnable(GL_TEXTURE_2D);  else functions.glDisable(GL_TEXTURE_2D);
    if (a_vert)  functions.glEnableClientState(GL_VERTEX_ARRAY);        else functions.glDisableClientState(GL_VERTEX_ARRAY);
    if (a_texc)  functions.glEnableClientState(GL_TEXTURE_COORD_ARRAY); else functions.glDisableClientState(GL_TEXTURE_COORD_ARRAY);
    if (a_col)   functions.glEnableClientState(GL_COLOR_ARRAY);         else functions.glDisableClientState(GL_COLOR_ARRAY);
    if (a_norm)  functions.glEnableClientState(GL_NORMAL_ARRAY);        else functions.glDisableClientState(GL_NORMAL_ARRAY);
    functions.glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, s_env);
    functions.glBindTexture(GL_TEXTURE_2D, (GLuint)s_tex);
    functions.glClientActiveTexture((GLenum)s_catex);
    functions.glActiveTexture((GLenum)s_atex);
}
...
my_glOrthof(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat zNear, GLfloat zFar)
{
    ...
    if(global_module_hacks.glOrthof_rotation_hack && apkenv_bound_fbo == 0)
    {
...
my_glBindFramebufferOES(GLenum target, GLuint framebuffer)
{
    ...
    if (global_module_hacks.render_to_fbo) {
        apkenv_fbo_ensure();
        if (framebuffer == 0) {
            /* engine wants the screen -> give it our offscreen portrait FBO */
            apkenv_bound_fbo = 0;
            functions.glBindFramebufferOES(target, apkenv_fbo_id);
            return;
        }
    }
